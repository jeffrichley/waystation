...
- **A failure is a value; only primitives raise** (ADR-0016). An awaited run returns `RunFailed`; a primitive raises `StageError` carrying the same `Failure`. `BaseException` never becomes a result: it is caught only to clean up or to hold a cancellation (ADR-0017), and always re-raised.
