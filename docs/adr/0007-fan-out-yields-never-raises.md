...
Leaving a bare `async for` early stops nothing, as with `asyncio.as_completed`: the runs left go on unobserved until they end, or until `asyncio.run` cancels them as it closes the loop, when each still keeps its work. Making the iterator an async generator whose close cancels was rejected: the close runs when the generator is collected, so the consumer cannot know when the batch has wound down — the uncertainty the block exists to remove. The block's exit, like `asyncio.TaskGroup`'s, returns only once every run it cancelled has wound down; a cancellation of the consumer meanwhile waits for that too and is raised afterwards.
