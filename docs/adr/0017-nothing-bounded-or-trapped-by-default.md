...
Shielding reaches past salvage: every stage's own work — the workspace clone, sandbox start, collect, integrate, teardown — runs to its end once begun, and a cancellation that arrives meanwhile is held and surfaces at the next stage boundary. Only the agent's exec and hooks are interrupted. A stage's own work is short next to an agent's, and waiting for it is what keeps a sandbox starting or tearing down from leaking. A stage's bound is the other way round: it kills the stage's work, host git included, and only a ref swap already under way is left to finish (ADR-0027). Once a cancellation is held no hook fires, not even `integrated` after a landing it let finish; the run's log line says where the series went instead, since no `run_end` carries a result.

The second signal's default is `SIG_DFL`, not Python's `KeyboardInterrupt`: an exception can be caught, or stall on joining the executor at shutdown, and the second signal exists to be an exit nothing can hold up — the line Go's `signal.NotifyContext` and Kubernetes' `SetupSignalHandler` draw. A signal that lands after the task has ended gets that default too, so a handler left installed with nothing to cancel never swallows a SIGTERM.

Decided in [wayfinder ticket 14](https://github.com/jeffrichley/waystation/issues/14), informed by the [sandcastle failure-semantics research](https://github.com/jeffrichley/waystation/issues/13). The stage-boundary hold and the meaning of "default" were settled in [#27](https://github.com/jeffrichley/waystation/issues/27).
