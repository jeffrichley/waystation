...
`_safely` has to be applied to each new `on_<hook>` a built-in observer grows; a forgotten one fails the run, which is loud rather than silent. The level is given back only if it is still the DEBUG that was taken, so a `configure_logging` call during the window wins. A run cancelled outright never fires `run_end`, so the handler stays attached and the level stays held for the life of the process; the console filter is what keeps that leak from being visible. [#27](https://github.com/jeffrichley/waystation/issues/27) left it standing: a cancelled run has no result for `run_end`, no hook fires once a cancellation is held (ADR-0017), and #18 rules out both a `Cancelled` result and sad-path hooks, so closing the file needs a decision of its own. It starts to matter with fan-out's early exit ([#31](https://github.com/jeffrichley/waystation/issues/31)), which cancels runs in a process that keeps going; #31 shipped that exit and left the leak for this decision.
