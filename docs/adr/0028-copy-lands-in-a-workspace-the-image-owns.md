...
- `clone_in` bundles the workspace branch alone. Extra refs (#32) must join the bundle when they arrive, or a copied sandbox will lack them. Until then a copied workspace has no remote, and a bound one keeps its clone's `origin`. **ADR-0037 amended this:** a workspace carries its `refs`, `clone_in` bundles and fetches every one of them, and `prepare_workspace` strips `origin` whatever the transport, so a bound workspace has no remote either.
...
Decided in [#37](https://github.com/jeffrichley/waystation/issues/37). Amended by [ADR-0037](0037-a-workspace-owns-its-branch-its-refs-and-its-removal.md): which refs travel, and that no remote does.
