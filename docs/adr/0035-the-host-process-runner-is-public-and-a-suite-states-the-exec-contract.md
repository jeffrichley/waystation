...
- `waystation.sandbox` gains `HostRunner` and `LineCallback`; top-level `waystation` gains nothing. `LineCallback` names a parameter of a protocol users implement. `allowlisted_env` was already there (ADR-0034). It also gained `discard_workspace`, which **ADR-0037 removed** along with the duty it named.
...
- `discard_workspace` was public because the contract then put the removal on the backend — `start` "owns `ws` from here" — and the commitment was made with its expiry stated rather than quietly. **That expiry arrived: ADR-0037** took the removal off backends, and the name and the conformance test that held the promise went with it. The suite now states the opposite contract, that a sandbox leaves the workspace where it found it.
