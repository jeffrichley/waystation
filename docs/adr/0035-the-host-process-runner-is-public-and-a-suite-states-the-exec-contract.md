...
- `waystation.testing` needs pytest, which waystation does not otherwise depend on: it is the `waystation[testing]` extra.
- The suite needs a POSIX `sh` in the sandbox and takes the argv that names one from a `shell` fixture, because a host path means nothing inside a container and a Windows host has no `sh` on PATH. #77 removes that knob by letting the sandbox answer.
