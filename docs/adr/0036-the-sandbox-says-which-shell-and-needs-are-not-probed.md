...
What a sandbox *otherwise* holds — git, base64, `sleep infinity`, a writable `/tmp`, the agent's own binary — stays written down rather than checked. When a shell cannot find the agent's command, exit 127 is logged as what it is: the sandbox is missing something, and waystation does not install it (ADR-0011).
...
Why `NoSandbox` preflights it: the backend that runs host processes is the one that needs a host sh, so it is the one that should say the host has none. That check used to live on `ScriptedAgent`, which is how a provider ended up caring which backend was about to run it. `preflight` now asks the host's git first, because a host with no git has no sh either and "install git" is the more useful of two true answers.
...
So the cost is paid late instead: a missing `claude` is `AgentExited(127)` after a sandbox has started. What this decision adds is that the 127 says so — it names what the shell could not find and points at ADR-0011, rather than leaving the reader to read an exit code. That is a smaller thing than the check, and it is the honest half.
...
- `Sandbox` gains a member, so every backend implements it and ADR-0035's conformance suite tests it. The suite lost the knob it had, which is a smaller public surface than before.
