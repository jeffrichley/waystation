...
Ignored files count as in the way. The fast-forward passes `--no-overwrite-ignore`, so git makes the same check again as it writes the checkout, and a file of either kind that appeared after waystation's last check is refused by git. That refusal still comes back as `dirty_tree`: when the fast-forward fails with the checkout unmoved, the checks run once more and name what got in the way. What remains is a file written while git itself is writing the checkout. `git merge` has that same window, and no library can close it. `merge` is a commit point beside `update-ref`: once started it finishes, and a cancellation is raised after it (ADR-0027).
