...
`RunFailed.stage` is `Stage | None`, so a `match` that reads it handles `None`. Only a queue produces it, and a run that began always names a stage. A run refused at preflight never began, so no hook fires for it and an observer never sees it, just as for a lone awaited run whose preflight raises. Only the queue's consumer sees it. Each queued run is preflighted alone, so a queue gives up the batch's check of each distinct agent and sandbox once. That is the cost of checking the host as it is when each run starts, and a caller who wants the batch's check preflights its specs itself and submits them `preflighted=True`. `waystation.__all__` gains `queue`, and `waystation.queue` exports `RunQueue` and `QueuedRun` as annotation aliases, as `waystation.stages` exports `StageRunner`. #18's import surface has to name `queue`. `CONTEXT.md` gains **Queue**. An ordering strategy for which queued run starts next is [#139](https://github.com/jeffrichley/waystation/issues/139). The default stays arrival order.
