...
- **One target per queue, depth 1, arrival order by default.** No speculation: a caller at this scale lands single digits in bursts, and Uber's follow-up (arXiv:2501.03440) cut CI use by speculating *less*. Which waiting candidate reaches the front next is `order=`, the same `OrderingStrategy` a run queue takes (ADR-0048), offered the waiting `QueuedCandidate` handles; a strategy that fails refuses the candidates it was ordering, each a `LandingFailed` whose `stage` is `None`.
...
- **The block** is `queue()`'s (ADR-0047): `submit` returns a handle whose `cancel()` stops that candidate alone; `close()` takes no more; leaving the block stops the candidate at the front, with its check's process tree killed and its workspace removed, and never starts the rest. The two queues share one private core, `_serving.Serving`, so the pull by an ordering strategy, the drain, the stop that waits and the cancelled wait that loses nothing are written once.
