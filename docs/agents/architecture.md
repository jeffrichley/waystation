...
- **Value Object** — results, `Timeouts` and the `Failure` union are frozen dataclasses that hold data and answer questions, with no behaviour worth mocking (ADR-0021). Two documented exceptions, each with an ADR: `CommandFailed` redacts itself (ADR-0025), and `Workspace` removes itself (ADR-0037) — a workspace is a resource as well as a value, and the alternative was leaving its removal to whichever party it was handed to. A third would mean this rule is not holding; argue it in an ADR or find the object it really belongs on.
