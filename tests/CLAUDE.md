...
| `MODULE_SURFACES` | every public module but the top level, with the names its `__all__` agrees to — `test_docs.py` holds each module to it and the site's reference has a page for each (#184); a new public module or name is a line here first |
...
| `until_batch(ready, results)` | `until` for a batch, whose run tasks a test never holds: it asks `results` for a result, and a result arriving before `ready()` holds *is* a run that ended first. Reach for it instead of `await some_event.wait()` inside a `fan_out`, `queue` or `merge_queue` block — a failure is a value (ADR-0016), so a run that dies before the point being waited for ends quietly and a bare wait on it hangs until CI kills the worker (#105) |
...
| `Stuck(repo, count)` | `count` runs that work until stopped (`runs()`), with `all_ready` set once all are working — stop them, then `assert_work_kept(temp)` proves each kept its work on its branch and left no workspace |
| `why(results)` | each failed result's stage and failure, as the message of an assertion over a batch's result types |
...
| `RecordingAgent()` | an agent that remembers each `prompt` and `schema` it was handed in `prompts` / `schemas`, then plays back a quiet `ScriptedAgent` — `plays=` swaps in another; one per test, since it keeps state across runs |
...
`ScriptedAgent` and `ScriptedCommit` come from `waystation.testing`, their one home (ADR-0044) — not `waystation`, which no longer exports them.
