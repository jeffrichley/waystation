...
| `until_batch(ready, results)` | `until` for a batch, whose run tasks a test never holds: it asks `results` for a result, and a result arriving before `ready()` holds *is* a run that ended first. Reach for it instead of `await some_event.wait()` inside a `fan_out` or `queue` block — a failure is a value (ADR-0016), so a run that dies before the point being waited for ends quietly and a bare wait on it hangs until CI kills the worker (#105) |
